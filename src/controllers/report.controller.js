...
import { sendCrimeReportConfirmationEmail, sendUpdatedCrimeReportEmail } from "../services/emailServices.js"
import path from 'path';
import { v4 as uuidv4 } from 'uuid';
import { generateReportPDF } from "../Utils/pdfGenerator.js";
import { fileURLToPath } from 'url';
import { crimeReportStatusTemplate, deleteComplaintTemplate, newComplaintTemplate, deleteAllComplaintTemplate, deleteSingleReportByAdminNotificationTemplate, deleteAllReportsByAdminNotificationTemplate,  updatedReportByAdminNotificationTemplate,  addedNewReportByAdminNotificationTemplate  } from "../Utils/notificationTemplates.js"; 
...
const createUserReport = asyncHandler(async (req, res) => {
    try {
        ...
        if (!prompt || typeof prompt !== "string" || prompt.trim().length === 0) {
            return ApiError(res, 400, "Prompt is required and must be a non-empty string!");
        }
        ...
        const sanitizedIncidentType = sanitizeString(aiResponse.data.incident_type.toLowerCase());
        ...
        if (evidenceFiles.length > 0) {
            ...
            const validEvidenceDocs = evidenceDocs.filter((evidence) => evidence !== null);
            if (validEvidenceDocs.length > 0) {
                report.evidences.push(...validEvidenceDocs.map((evidence) => evidence._id));
                await report.save();
            }
        }

        // Step 8: Generate and upload the PDF (unchanged)
        const pdfFilePath = path.join(__dirname, "../../public/temp/", `Crime_Report${report.caseNumber}.pdf`);
        ...
        const { title, body } = newComplaintTemplate(report.caseNumber, report.status);
        const userToken = user.fcmToken; 
        const data = { caseNumber: report.caseNumber, severity, reportId: report._id };

        await sendNotifications(user._id, userToken, adminTokens, title, body, data, severity, "both");
...
const getSingleUserReport = asyncHandler(async (req, res) => {
    ...
    const report = await Reports.findOne({  userId: userId, _id: reportId, deletedByUser: false })
        .populate('userId', 'fullname email NICNumber') 
    ...
    try {
        ...
        try {
            ...
            for (const report of reports) {
                ...
                const data = { caseNumber: report.caseNumber, reportId: report._id, userName: user.fullname };
                await sendNotifications(userId, userToken, [], title, body, data, "user");
            }
        } catch (error) {
        ...
        return res.status(200).json(new ApiResponse(200, {}, "All reports deleted successfully"));
    } catch (error) {
...
const checkStatus = asyncHandler(async (req, res) => {
    ...
    try {
        const report = await Reports.findOne({  
            ...
            userId: userId,
            deletedByUser: false,
        }).select("reportStatus reportedDate incident_type caseNumber policeStationName");
...
const getSingleAdminReport = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const report = await Reports.findById(id)
            .populate('userId', 'fullname email NICNumber'); // Populate user details
...
const deleteAdminReport = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Send notifications
        const { title, body } = deleteSingleReportByAdminNotificationTemplate(caseNumber);
        const userToken = req.user.fcmToken; 
        const data = { caseNumber, reportId: id };
        const adminTokens = getAdminTokens();
        await sendNotifications(userId, userToken, adminTokens, title, body, data, "admin");
...
const deleteAllAdminReports = asyncHandler(async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, {}, `${deleteResult.deletedCount} reports deleted successfully`));
    } catch (error) {
...
const updateReportStatus = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Send notifications 
        const adminTokens = await getAdminTokens();
        const { title, body } = crimeReportStatusTemplate(updatedReportStatus.caseNumber, updatedReportStatus.reportStatus);

        const data = { caseNumber: updatedReportStatus.caseNumber, reportId: updatedReportStatus._id };

        console.log(crimeSeverity[updatedReportStatus.incident_type] || defaultSeverity);
        
        await sendNotifications(updatedReportStatus.userId._id, userToken, [], title, body, data, crimeSeverity[updatedReportStatus.incident_type] || defaultSeverity);

        await sendUpdatedCrimeReportEmail(userFullName, userEmail, updatedReportStatus.caseNumber, updatedReportStatus.reportStatus);

        return res
            .status(200)
            .json(new ApiResponse(200, updatedReportStatus, "Report status updated successfully"));
    } catch (error) {
...
const addNewAdminReport = asyncHandler(async (req, res) => {
    try {
        const { complainant_name, complainant_email, complainant_nic, incident_type, incident_description, location, reportStatus, policeStationName } = req.body;
        const signatureImage = req.files?.signatureImage?.[0]?.path;
        const evidenceFiles = req.files?.evidenceFiles || [];

        if (!complainant_name || !complainant_nic || !complainant_email || !incident_type || !incident_description || !location || !policeStationName || !reportStatus || !signatureImage) {
        ...
        await sendCrimeReportConfirmationEmail(complainant_name, complainant_email, report.caseNumber);

        // Send notification to the user and admins
        const { title, body } = addedNewReportByAdminNotificationTemplate(report.caseNumber);
...
const updateAdminReport = asyncHandler(async (req, res) => {
    try {
        const { id } = req.params;
        const { nic, incident_description, incident_type, policeStationName, location, complainant_name, complainant_email } = req.body;
        const signatureImage = req.files?.signatureImage?.[0]?.path;
        ...
        if (complainant_name && complainant_name.trim() !== existingReport.complainant_name)
            updateFields.complainant_name = complainant_name.trim();
        if (complainant_email && complainant_email.trim() !== existingReport.complainant_email)
            updateFields.complainant_email = complainant_email.trim();
        if (incident_description && incident_description.trim() !== existingReport.incident_description)
        ...
        if (policeStationName && policeStationName !== existingReport.policeStationName)
        ...
        const mergedReportData = {
            ...
            policeStationName: updatedReport.policeStationName || existingReport.policeStationName,
            incident_type: updatedReport.incident_type || existingReport.incident_type,
            ...
            email: updatedReport.complainant_email || existingReport.complainant_email,
            ...
            incident_description: updatedReport.incident_description || existingReport.incident_description,
        };

        // Generate a new PDF using the merged data and signature image
        const pdfFilePath = path.join(__dirname, "../../public/temp/", `Crime_Report${updatedReport.caseNumber}.pdf`);
        await generateReportPDF(
            mergedReportData,
        ...
        // Send notification to the user and admins
        const { title, body } = updatedReportByAdminNotificationTemplate(updatedReport.caseNumber);
        const userToken = req.user.fcmToken; // FCM token of the user who created the report
        const adminTokens = getAdminTokens(); // FCM tokens of all admins
        const data = { caseNumber: updatedReport.caseNumber, reportId: updatedReport._id };
